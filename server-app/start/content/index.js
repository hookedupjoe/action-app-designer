...
module.exports.setup = function setup(scope) {
    ...
    return $.async(function processReq(req, res, next) {
        ...
        try {
            //--- Assure content setup object is laoded one time
            var tmpAppSetup = $.await(scope.locals.$.AppUtils.getAppSetup());
            var tmpAppAreaName = scope.locals.path.content  + '/' + tmpAppAreaName + '.js';
            ...
            if (typeof(tmpAppReq.setup) == 'function') {
                ...
                res.json({status:false, error: "Could not find content area " + tmpAppAreaName})
                return
            }
        } catch (ex) {
